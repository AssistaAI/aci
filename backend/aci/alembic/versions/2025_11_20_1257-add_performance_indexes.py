...
def upgrade() -> None:
    ...
    op.create_index('ix_apps_active', 'apps', ['active'])
    op.create_index('ix_apps_visibility_active', 'apps', ['visibility', 'active'])

    # Functions table indexes for filtering and searching
    op.create_index('ix_functions_visibility', 'functions', ['visibility'])
    op.create_index('ix_functions_active', 'functions', ['active'])
    op.create_index('ix_functions_app_id', 'functions', ['app_id'])
    op.create_index('ix_functions_visibility_active', 'functions', ['visibility', 'active'])
    op.create_index('ix_functions_app_id_visibility_active', 'functions', ['app_id', 'visibility', 'active'])

    # LinkedAccounts table indexes for project-scoped queries
    op.create_index('ix_linked_accounts_project_id', 'linked_accounts', ['project_id'])
    op.create_index('ix_linked_accounts_app_id', 'linked_accounts', ['app_id'])
    op.create_index('ix_linked_accounts_project_app', 'linked_accounts', ['project_id', 'app_id'])
    op.create_index('ix_linked_accounts_owner_id', 'linked_accounts', ['linked_account_owner_id'])
    op.create_index('ix_linked_accounts_enabled', 'linked_accounts', ['enabled'])

    # AppConfigurations table indexes for project-scoped queries
    op.create_index('ix_app_configurations_project_id', 'app_configurations', ['project_id'])
    op.create_index('ix_app_configurations_app_id', 'app_configurations', ['app_id'])
    op.create_index('ix_app_configurations_enabled', 'app_configurations', ['enabled'])
